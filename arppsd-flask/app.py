...
def run_diagnostics():
    ...
    if on_render and not db.DB_PATH.startswith("/var/data"):
        issues.append(dict(
            title="Base de données non persistante",
            detail="Sans disque persistant, la base de données sera effacée à chaque "
                   "redéploiement. Ajoutez un disque Render monté sur /var/data et "
                   "définissez la variable DB_PATH=/var/data/arppsd.db.",
        ))
    if on_render and not UPLOAD_FOLDER.startswith("/var/data"):
        issues.append(dict(
            title="Documents non persistants",
            detail="Les documents téléversés seront effacés à chaque redéploiement. "
                   "Définissez UPLOAD_FOLDER=/var/data/uploads sur le disque persistant.",
        ))
...
def document_delete(did):
    name, stored_filename = db.delete_document(did)
    if stored_filename:
