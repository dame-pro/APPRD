...
def insert_document(name, category, note, added_by, filename=None, original_name=None, file_size=None):
    db = get_db()
    db.execute(
        "INSERT INTO document (id, name, category, note, date_added, added_by, is_demo, "
        "filename, original_name, file_size) VALUES (?,?,?,?,?,?,0,?,?,?)",
...
def delete_document(doc_id):
    db = get_db()
    row = db.execute("SELECT name, filename FROM document WHERE id=?", (doc_id,)).fetchone()
    name = row["name"] if row else ""
    stored_filename = row["filename"] if row else None
    db.execute("DELETE FROM document WHERE id=?", (doc_id,))
    db.commit()
    return name, stored_filename
